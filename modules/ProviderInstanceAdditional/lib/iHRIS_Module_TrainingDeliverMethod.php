...
class iHRIS_Module_TrainingDeliverMethod extends I2CE_Module {

  
    public static function getMethods() {
        return array(
            'iHRIS_PageView->action_training_deliver_method' => 'action_training_deliver_method'
            );
    }


    public function action_training_deliver_method($obj) {
        if (!$obj instanceof iHRIS_PageView) {
            return;
        }
        return $obj->addChildForms('training_deliver_method', 'siteContent');
    }
    ...
    protected function pre_migrate() {
        ...
        I2CE_FormStorage::storeMigrateData( array( "provider_instance" => array( "training_deliver_method" ) ), $migrate_path );
    ...
    protected function migrate() {
        ...
        $migrate_path = "/I2CE/formsData/migrate_data/4.1.8";
        if ( !I2CE_FormStorage::migrateForm( "training_deliver_method", "entry", $user, $migrate_path ) ) {
            return false;
        }
        if ( I2CE_ModuleFactory::instance()->isEnabled("CachedForms") ) {
            $cachedForm = new I2CE_CachedForm( "training_deliver_method" );
            $cachedForm->dropTable();
        }
        if ( !I2CE_FormStorage::migrateField( "provider_instance", array( "training_deliver_method" => "training_deliver_method" ),
                    $migrate_path, $user ) ) {
            return false;
        }
        return true;
