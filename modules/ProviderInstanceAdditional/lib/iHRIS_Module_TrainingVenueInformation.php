...
class iHRIS_Module_TrainingVenueInformation extends I2CE_Module {

  
    public static function getMethods() {
        return array(
            'iHRIS_PageView->action_training_venue_information' => 'action_training_venue_information'
            );
    }


    public function action_training_venue_information($obj) {
        if (!$obj instanceof iHRIS_PageView) {
            return;
        }
        return $obj->addChildForms('training_venue_information', 'siteContent');
    }
    ...
    protected function pre_migrate() {
        ...
        I2CE_FormStorage::storeMigrateData( array( "provider_instance" => array( "training_venue_information" ) ), $migrate_path );
    ...
    protected function migrate() {
        ...
        $migrate_path = "/I2CE/formsData/migrate_data/4.1.8";
        if ( !I2CE_FormStorage::migrateForm( "training_venue_information", "entry", $user, $migrate_path ) ) {
            return false;
        }
        if ( I2CE_ModuleFactory::instance()->isEnabled("CachedForms") ) {
            $cachedForm = new I2CE_CachedForm( "training_venue_information" );
            $cachedForm->dropTable();
        }
        if ( !I2CE_FormStorage::migrateField( "provider_instance", array( "training_venue_information" => "training_venue_information" ),
                    $migrate_path, $user ) ) {
            return false;
        }
        return true;
