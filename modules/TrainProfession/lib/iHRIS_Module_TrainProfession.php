...
class iHRIS_Module_TrainProfession extends I2CE_Module {

  
    public static function getMethods() {
        return array(
            'iHRIS_PageView->action_train_profession' => 'action_train_profession'
            );
    }


    public function action_training_area($obj) {
        if (!$obj instanceof iHRIS_PageView) {
            return;
        }
        return $obj->addChildForms('train_profession', 'siteContent');
    }
    ...
    protected function pre_migrate() {
        ...
        I2CE_FormStorage::storeMigrateData( array( "position" => array( "train_profession" ) ), $migrate_path );
    ...
    protected function migrate() {
        ...
        $migrate_path = "/I2CE/formsData/migrate_data/4.1.8";
        if ( !I2CE_FormStorage::migrateForm( "train_profession", "entry", $user, $migrate_path ) ) {
            return false;
        }
        if ( I2CE_ModuleFactory::instance()->isEnabled("CachedForms") ) {
            $cachedForm = new I2CE_CachedForm( "train_profession" );
            $cachedForm->dropTable();
        }
        if ( !I2CE_FormStorage::migrateField( "position", array( "train_profession" => "train_profession" ),
                    $migrate_path, $user ) ) {
            return false;
        }
        return true;
