...
class iHRIS_Module_AcademicInstitution extends I2CE_Module {

  
    public static function getMethods() {
        return array(
            'iHRIS_PageView->action_academic_institution' => 'action_academic_institution'
            );
    }


    public function action_academic_institution($obj) {
        if (!$obj instanceof iHRIS_PageView) {
            return;
        }
        return $obj->addChildForms('academic_institution', 'siteContent');
    }
    ...
    protected function pre_migrate() {
        ...
        I2CE_FormStorage::storeMigrateData( array( "education" => array( "academic_institution" ) ), $migrate_path );
    ...
    protected function migrate() {
        ...
        $migrate_path = "/I2CE/formsData/migrate_data/4.1.8";
        if ( !I2CE_FormStorage::migrateForm( "academic_institution", "entry", $user, $migrate_path ) ) {
            return false;
        }
        if ( I2CE_ModuleFactory::instance()->isEnabled("CachedForms") ) {
            $cachedForm = new I2CE_CachedForm( "academic_institution" );
            $cachedForm->dropTable();
        }
        if ( !I2CE_FormStorage::migrateField( "education", array( "academic_institution" => "academic_institution"),
                    $migrate_path, $user ) ) {
            return false;
        }
        return true;
