...
*/
/**
* Class iHRIS_Module_FUATResult
*
* @access public
*/


class iHRIS_Module_FUATResult extends I2CE_Module {

  
    public static function getMethods() {
        return array(
            'iHRIS_PageView->action_fuat_result' => 'action_fuat_result'
            );
    }


    public function action_fuat_result($obj) {
        if (!$obj instanceof iHRIS_PageView) {
            return;
        }
        return $obj->addChildForms('fuat_result', 'siteContent');
    }
    ...
    protected function pre_migrate() {
        ...
        I2CE_FormStorage::storeMigrateData( array( "training_fuat" => array( "fuat_result" ) ), $migrate_path );
    ...
    protected function migrate() {
        ...
        $migrate_path = "/I2CE/formsData/migrate_data/4.1.8";
        if ( !I2CE_FormStorage::migrateForm( "fuat_result", "entry", $user, $migrate_path ) ) {
            return false;
        }
        if ( I2CE_ModuleFactory::instance()->isEnabled("CachedForms") ) {
            $cachedForm = new I2CE_CachedForm( "fuat_result" );
            $cachedForm->dropTable();
        }
        if ( !I2CE_FormStorage::migrateField( "training_fuat", array( "fuat_result" => "fuat_result" ),
                    $migrate_path, $user ) ) {
            return false;
        }
        return true;
