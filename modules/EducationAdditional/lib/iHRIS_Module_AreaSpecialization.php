...
class iHRIS_Module_AreaSpecialization extends I2CE_Module {

  
    public static function getMethods() {
        return array(
            'iHRIS_PageView->action_area_specialization' => 'action_area_specialization'
            );
    }


    public function action_job_category($obj) {
        if (!$obj instanceof iHRIS_PageView) {
            return;
        }
        return $obj->addChildForms('area_specialization', 'siteContent');
    }
    ...
    protected function pre_migrate() {
        ...
        I2CE_FormStorage::storeMigrateData( array( "education" => array( "area_specialization" ) ), $migrate_path );
    ...
    protected function migrate() {
        ...
        $migrate_path = "/I2CE/formsData/migrate_data/4.1.8";
        if ( !I2CE_FormStorage::migrateForm( "area_specialization", "entry", $user, $migrate_path ) ) {
            return false;
        }
        if ( I2CE_ModuleFactory::instance()->isEnabled("CachedForms") ) {
            $cachedForm = new I2CE_CachedForm( "area_specialization" );
            $cachedForm->dropTable();
        }
        if ( !I2CE_FormStorage::migrateField( "education", array( "area_specialization" => "area_specialization" ),
                    $migrate_path, $user ) ) {
            return false;
        }
        return true;
