...
class iHRIS_Module_JobCategory extends I2CE_Module {

  
    public static function getMethods() {
        return array(
            'iHRIS_PageView->action_job_category' => 'action_job_category'
            );
    }
    ...
    public function action_job_category($obj) {
        ...
        return $obj->addChildForms('job_category', 'siteContent');
    }
    ...
    protected function pre_migrate() {
        ...
        I2CE_FormStorage::storeMigrateData( array( "position" => array( "job_category" ) ), $migrate_path );
    ...
    protected function migrate() {
        ...
        $migrate_path = "/I2CE/formsData/migrate_data/4.1.8";
        if ( !I2CE_FormStorage::migrateForm( "job_category", "entry", $user, $migrate_path ) ) {
            return false;
        }
        if ( I2CE_ModuleFactory::instance()->isEnabled("CachedForms") ) {
            $cachedForm = new I2CE_CachedForm( "job_category" );
            $cachedForm->dropTable();
        }
        if ( !I2CE_FormStorage::migrateField( "position", array( "job_category" => "job_category" ),
                    $migrate_path, $user ) ) {
            return false;
        }
        return true;
